	...
	implements Iface, \Aimeos\Controller\Frontend\Common\Iface
{
	...
	/**
	 * Adds keyword IDs for filtering
	 *
	 * @param array|string $keyIds Supplier ID or list of IDs
	 * @param string $listtype List type of the products referenced by the keywords
	 * @return \Aimeos\Controller\Frontend\Product\Iface Product controller for fluent interface
	 * @since 2019.04
	 */
	public function keyword( $keyIds, string $listtype = 'default' ) : Iface
	{
        
		if( !empty( $keyIds ) && ( $ids = array_unique( $this->validateIds( (array) $keyIds ) ) ) !== [] )
		{
			
			
			$this->addExpression( $this->filter->compare( '==', 'index.keyword.id', $ids ) );
