 ...
 * @subpackage Html
 */
class Catsuggestnew extends Standard
{
	public function data( \Aimeos\Base\View\Iface $view, array &$tags = [], string &$expire = null ) : \Aimeos\Base\View\Iface
	{
		...
		$keywordItems = \Aimeos\Controller\Frontend::create( $context, 'keyword' )->uses( $domains )
			...
			->slice( 0, $size - count( $catItems ) )
			->search();


		$cntl = \Aimeos\Controller\Frontend::create( $context, 'product' )->uses( $domains )
			// ->text( $text )
			->compare( "=~" , "product.label" ,$text  ); // sort by relevance first
		...
		$view->suggestItems = $cntl->slice( 0, $size - count( $catItems ) - count( $supItems ) -count($keywordItems) )->search();
		
		$view->suggestSupplierItems = $supItems;
		$view->suggestCatalogItems = $catItems;
		$view->suggestKeywordItems = $keywordItems;
