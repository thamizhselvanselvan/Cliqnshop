	...
	implements \Aimeos\Client\Html\Common\Client\Factory\Iface
{
	...
	public function data( \Aimeos\Base\View\Iface $view, array &$tags = [], string &$expire = null ) : \Aimeos\Base\View\Iface
	{
		...
		// storing search term data logic goes here
		// if(strlen($view->param( 'f_search' ))>=3 && $total <= 0)
		// {
		...
		$response =	Http::get('https://catalog-manager-mosh.com/api/product', [
			'search' => $request,
			// 'siteId' => $site,
			'source' => $sitecode,
		]);
	...
	if (app()->environment() === 'production') {
		$response =	Http::get('https://app.360ecom.io/api/product', [
			'search' => $request,
			// 'siteId' => $site,
			'source' => $sitecode,
		]);
		...
		// }
		// storing search term data logic ends  here
...
// dd($this->view()->param());
	...
	/**
	 * Returns the keyword IDs used for filtering products
	 *
	 * @return array List of keyword IDs
	 */
	protected function keywords() : array
	{
		/** client/html/catalog/lists/supid-default
		 * The default keyword ID used if none is given as parameter
		 *
		 * Products in the list page can be limited to one or more keywords.
		 * By default, the products are not limited to any keyword until one or
		 * more keyword IDs are passed in the URL using the f_supid parameter.
		 * You can also configure the default keyword IDs for limiting the
		 * products if no IDs are passed in the URL using this configuration.
		 *
		 * @param array|string Keyword ID or IDs
		 * @since 2021.01
		 * @see client/html/catalog/lists/sort
		 * @see client/html/catalog/lists/size
		 * @see client/html/catalog/lists/domains
		 ...
		 * @see client/html/catalog/instock
		 */
		$keyids = $this->view()->param( 'f_keyid', $this->context()->config()->get( 'client/html/catalog/lists/keyid-default' ) );
		$keyids = $keyids != null && is_scalar( $keyids ) ? explode( ',', $keyids ) : $keyids; // workaround for TYPO3
