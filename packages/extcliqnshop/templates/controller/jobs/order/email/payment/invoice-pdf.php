...
'
		...
		<td style="width: 33%">
			<table class="meta">
				<tr>
					<td><?= $enc->html( $this->translate( 'controller/jobs', 'Order date' ) ) ?>:</td>
					<td><?= $enc->html( date_create( $this->orderItem->getTimeCreated() )->format( $this->translate( 'controller/jobs', 'Y-m-d' ) ) ) ?></td>
				</tr>
				<?php if( $this->summaryBasket->getCustomerReference() ) : ?>
					<tr>
						<td><?= $enc->html( $this->translate( 'controller/jobs', 'Reference' ) ) ?>:</td>
						<td><?= $enc->html( $this->summaryBasket->getCustomerReference() ) ?></td>
					</tr>
				<?php endif ?>
				<tr>
					<td colspan="2"></td>
