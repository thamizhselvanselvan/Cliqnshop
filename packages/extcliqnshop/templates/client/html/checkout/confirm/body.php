...
<section class="aimeos checkout-confirm" data-jsonurl="<?= $enc->attr( $this->link( 'client/jsonapi/url' ) ) ?>">
	<div class="container">
		...
		<h1><?= $enc->html( $this->translate( 'client', '' ), $enc::TRUST ) ?></h1>
		...
		<div class="checkout-confirm-intro">
			<?php switch( $this->confirmOrderItem->getStatusPayment() ) : case \Aimeos\MShop\Order\Item\Base::PAY_CANCELED: ?>
				<div>
					<div class="check pay-canceled">
						<i class="fa fa-times-circle-o" aria-hidden="true"></i>
					</div>
					<h3 class="noteheader">
					<?= nl2br( $enc->html( $this->translate( 'client', "Order Canceled !" ), $enc::TRUST ) );  ?>
					</h3>
					<p class="note">
						<?= nl2br( $enc->html( $this->translate( 'client', "The order was canceled.
			...
			<?php case \Aimeos\MShop\Order\Item\Base::PAY_REFUSED: ?>
				<div>
					<div class="check pay-refused">
						<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
					</div>
					<h3 class="noteheader">
					<?= nl2br( $enc->html( $this->translate( 'client', "Payment refused !" ), $enc::TRUST ) );  ?>
					</h3>
					<p class="note">
						<?= nl2br( $enc->html( $this->translate( 'client', "Unfortunately, the payment for your order was refused.
							Do you wish to retry?" ), $enc::TRUST ) );  ?>
					</p>
				</div>
				<?php break ?>
			...
			<?php case \Aimeos\MShop\Order\Item\Base::PAY_PENDING: ?>
				<div>
					<div class="check pay-pending">
						<i class="fa Example of clock-o fa-clock-o" aria-hidden="true"></i>
					</div>
					<h3 class="noteheader">
						<?= nl2br( $enc->html( $this->translate( 'client', "Order Pending." ), $enc::TRUST ) );  ?>
					</h3>
					<p class="note">
						<?= nl2br( $enc->html( $this->translate( 'client', "The payment confirmation for your order is still pending.
						You will get an e-mail as soon as the payment is authorized." ), $enc::TRUST ) );  ?>
					</p>
				</div>
				<?php break ?>
			...
			<?php case \Aimeos\MShop\Order\Item\Base::PAY_AUTHORIZED: ?>
				<div>
					<div class="check pay-authorised">
						<i class="fa fa-check-circle-o" aria-hidden="true"></i>
					</div>
					<h3 class="noteheader">
					<?= nl2br( $enc->html( $this->translate( 'client', "Thank you for your order and authorizing the payment." ), $enc::TRUST ) );  ?>
					</h3>
					<p class="note">
						<?= nl2br( $enc->html( $this->translate( 'client', "An e-mail with the order details will be sent to you within the next few minutes." ), $enc::TRUST ) );  ?>
					</p>
				</div>
				<?php break ?>
			<?php case \Aimeos\MShop\Order\Item\Base::PAY_RECEIVED: ?>								
				<div>
					<div class="check pay-recieved">
						<i class="fa fa-check-circle-o" aria-hidden="true"></i>
					</div>
					<h3 class="noteheader">
					<?= nl2br( $enc->html( $this->translate( 'client', "Thank you for your order." ), $enc::TRUST ) );  ?>
					</h3>
					<p class="note">
						<?= nl2br( $enc->html( $this->translate( 'client', "We received your payment and an e-mail with the order details will be sent to you within the next few minutes." ), $enc::TRUST ) );  ?>
					</p>
				</div>
				<?php break ?>
			<?php endswitch ?>
		...
		<div class="checkout-confirm-basic">
			<h2 class="mx-0"><?= $enc->html( $this->translate( 'client', 'Order status' ), $enc::TRUST ) ?></h2>
		...
		<div class="checkout-confirm-detail common-summary">
			...
			<div class="common-summary-detail row">
				...
				<div class="basket col-sm-12 px-0">
					<?= $this->partial(
