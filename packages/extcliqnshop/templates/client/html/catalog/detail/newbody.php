...
<!-- breadcrumb template added by client -->
<!-- catalog.detail.breadcrumb -->
    ...
    ['mediaItems' => $this->get('detailMediaItems', map()), 'params' => $this->param()]
				...
				<div class="col-sm-6">
					...
					<div class="catalog-detail-basic">
						...
						<!-- code to show the product direct link to amazon --start -->
						<?php
							$context = app('aimeos.context')->get(false);
							$locale = $context->locale();
							$site = ($locale)->getSiteItem()->getCode();
							$siteid = ($locale)->getSiteItem()->getSiteid();
							$code = $this->detailProductItem->getCode();
						?>
							<?php if (App::environment(['local', 'staging'])) : ?>
								<?php if ($site=='in') : ?>
									<?php $asin = DB::table('mshop_product')->where('siteid',$siteid)->where('code', $code)->pluck('asin')->ToArray();?>
									<p class="code" >
										<a style= "color: #af6554; font-size: 12px; text-decoration: underline;" target="_blank"  href="https://www.amazon.com/dp/<?=$enc->html($asin[0])?>">View Product On Amazon</a>
									</p>
								<?php else : ?>
									<?php $asin = DB::table('mshop_product')->where('siteid',$siteid)->where('code', $code)->pluck('asin')->ToArray();?>
									<p class="code" >
										<a style= "color: #af6554; font-size: 12px; text-decoration: underline;" target="_blank"  href="https://www.amazon.ae/dp/<?=$enc->html($asin[0])?>">View Product On Amazon</a>
									</p>
								<?php endif?>
							<?php endif?>
							<!-- code to show the product direct link to amazon --end -->
							...
							<?php if (!$this->detailProductItem->getRefItems('price', 'default', 'default')->empty()): ?>
								<div class="addbasket">
									...
									<div class="input-group">
										...
										<?php	$context = app('aimeos.context')->get(false);
$locale = $context->locale();
$site = ($locale)->getSiteItem()->getCode();
if ($site == 'in'): ?>
												<mark class="mt-2 col-sm-4 text-danger">Required KYC for this Product</mark>
												<?php endif?>
									</div>
								</div>
							<?php endif?>
